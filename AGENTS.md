...
3. `review-platform` / `ops`를 제품 대체물이 아니라 contract와 lifecycle을 검증하는
   harness로 유지한다.
...
- `review-platform/`
  ...
  - 운영 UI가 아니라 bot/API/adapter contract를 재현하고 검증하는 장치
- `ops/`
...
5. `review-platform` 또는 `ops` harness 변경이면 아래를 같이 확인한다
   ...
   - [review-platform/tests/test_pr_flow.py](/home/et16/work/review_system/review-platform/tests/test_pr_flow.py:1)
   - [review-bot/tests/test_multilang_smoke_fixture.py](/home/et16/work/review_system/review-bot/tests/test_multilang_smoke_fixture.py:1)
   - [ops/fixtures/review_smoke/README.md](/home/et16/work/review_system/ops/fixtures/review_smoke/README.md:1)
6. local compose를 쓸지, 단위 테스트만 쓸지 먼저 결정한다
...
- local harness는 legacy `pr_id` bot API를 되살리는 곳이 아닙니다
  - current-state bot 연동은 `ReviewRequestKey` 기반 API 또는 runner-level helper를 사용해야 합니다
...
하네스 작업은 "사용자 기능을 하나 더 만든다"보다 "운영 contract를 더 재현 가능하게 만든다"를
우선 기준으로 봅니다.

- `review-platform`은 Git review system의 canonical UI가 아니라 local adapter/harness입니다
  - product 판단의 근거가 필요하면 GitLab MR surface와 `review-bot` API contract를 먼저 봅니다
- `ops` smoke는 단순 end-to-end 데모가 아니라 lifecycle invariant 검증입니다
  - baseline review, incremental replay, thread reply, resolve, sync, analytics 증가를 각각 신호로 봅니다
- harness fixture는 작고 의도적인 regression specimen이어야 합니다
  - `expected_smoke.json`은 language tag, routing, density, wrong-language feedback contract만 담습니다
  - detector miss와 fixture/policy mismatch를 섞지 않습니다
- harness가 운영 behavior를 흉내낼 때도 identity는 항상 canonical key를 따릅니다
  - `review_system`, `project_ref`, `review_request_id`가 로그, artifact, API 요청에서 서로 맞는지 봅니다
- harness 테스트에서 mock을 쓰더라도 drift 방지용 unmocked contract 확인을 하나는 남기는 편이 좋습니다
  - 특히 bot bridge, full-report/backlog view, incremental diff `base_sha`는 drift가 나기 쉽습니다
...
- local harness bot bridge / tests
  - [review-platform/app/clients/bot_client.py](/home/et16/work/review_system/review-platform/app/clients/bot_client.py:1)
  - [review-platform/tests/test_pr_flow.py](/home/et16/work/review_system/review-platform/tests/test_pr_flow.py:1)
- smoke fixture contracts
  - [ops/fixtures/review_smoke/README.md](/home/et16/work/review_system/ops/fixtures/review_smoke/README.md:1)
  - [ops/scripts/smoke_local_gitlab_multilang_review.py](/home/et16/work/review_system/ops/scripts/smoke_local_gitlab_multilang_review.py:1)
  - [review-bot/tests/test_multilang_smoke_fixture.py](/home/et16/work/review_system/review-bot/tests/test_multilang_smoke_fixture.py:1)

## 6. Runtime Ports And Services
...
harness fixture 또는 multilang smoke contract 변경 시:
...
## 8. Phase A Trust Foundation Summary
...
## 9. GitLab-Specific Gotchas

최근에 실제로 다시 확인할 가치가 높았던 항목입니다.

### 9.1 Note Hook head race
...
## 10. Harness-Specific Gotchas

### 10.1 local harness bot bridge drift

current-state contract에서는 legacy `pr_id` endpoint가 제거되었습니다.
`review-platform`의 bot trigger/state UI를 건드릴 때는 아래를 먼저 확인합니다.

- `review-platform/app/clients/bot_client.py`가 `/internal/review/runs`와
  `/internal/review/requests/{review_system}/{project_ref}/{review_request_id}` 계열을 쓰는지 확인
- `review-platform/tests/test_pr_flow.py`가 stale bridge를 mock으로만 고정하지 않는지 확인
- 필요하면 unsupported harness control을 숨기거나, key-based next-batch contract를 먼저 문서화합니다

관련 현재 리뷰 backlog:

- [docs/reviews/REVIEW_FINDINGS_BACKLOG.md](/home/et16/work/review_system/docs/reviews/REVIEW_FINDINGS_BACKLOG.md:37)

### 10.2 fixture contract와 detector fix 구분

mixed-language smoke fixture가 만든 wrong-language reply는 feedback loop 검증용입니다.
운영 detector backlog로 바로 옮기지 말고 아래 순서로 분리합니다.

1. `expected_smoke.json`의 expected/forbidden language tag와 density contract가 맞는지 확인
2. `policy_mismatch` 또는 `update_policy_or_fixture`면 fixture나 reviewability policy를 먼저 수정
3. production project에서 반복되는 `actionability=fix_detector` 후보만 detector backlog로 승격

### 10.3 smoke artifact 해석

local GitLab lifecycle smoke 성공은 provider direct 성공을 의미하지 않습니다.
하네스 관점에서는 lifecycle invariant pass와 provider direct pass를 별도 신호로 기록합니다.
...
## 12. Last Known Good Validation Path

다음 경로는 실제로 끝까지 성공한 표준 검증 흐름입니다.

- `review-bot` 핵심 테스트 4종
- `review-platform` 테스트
- `review-bot` multilang smoke fixture contract 테스트
...
- harness fixture contract나 local bridge contract가 바뀌었을 때
