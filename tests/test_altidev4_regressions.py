...
def _load_manifest() -> list[dict[str, object]]:
    manifest_path = Path("examples/altidev4_snippets.json")
...
def test_altidev4_snippet_manifest_points_to_existing_non_empty_examples() -> None:
    ...
    for item in manifest:
        ...
        assert not example_path.is_absolute(), f"Snippet path must be repo-local: {example_path}"


def test_altidev4_queue_perf_patterns_are_detected_from_local_snippet() -> None:
    queue_perf = _read_text(Path("examples/altidev4/queue_perf_memory_and_rc.cpp"))
...
def test_altidev4_stackwalker_excerpt_detects_switch_and_free_patterns() -> None:
    excerpt = _read_text(Path("examples/altidev4/stackwalker_switch_and_free.cpp"))
