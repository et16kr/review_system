...
local backend capture env contract:

- default OpenAI direct artifact는 `BOT_OPENAI_BASE_URL`을 비우거나 unset 한 상태에서만 남긴다.
- local backend artifact는 `BOT_OPENAI_BASE_URL=<non-default OpenAI-compatible /v1 URL>`과
  `BOT_OPENAI_MODEL=<backend model>`을 명시하고, backend가 auth를 무시하더라도
  `OPENAI_API_KEY=<placeholder accepted by backend>`를 명시한다.
- fallback lifecycle 확인이 필요하면 `BOT_FALLBACK_PROVIDER=stub`를 유지할 수 있지만,
  이 값은 direct smoke/provider-quality artifact의 backend success 판정에 쓰지 않는다.
- retained artifact 본문이나 JSON의 `provider_runtime.endpoint_base_url`,
  `provider_runtime.configured_model`, `provider_runtime.transport_class`가 실행 env와 다르면
  그 artifact는 tuning evidence가 아니라 diagnostic으로만 본다.
...
retained direct smoke filename:

- default OpenAI: `docs/baselines/review_bot/direct_openai_smoke_YYYY-MM-DD.txt`
- local backend: `docs/baselines/review_bot/direct_openai_smoke_openai_compatible_local_YYYY-MM-DD.txt`

direct provider success artifact로 쓰려면 `--expect-live-openai`를 붙이고, exit `0`,
`models_probe_status=ok`, `live_probe_model=...`가 함께 남아야 한다. `insufficient_quota`,
transport timeout, missing key, 또는 `--expect-live-openai` 없이 끝난 run은 diagnostic artifact일 수는
있지만 provider/ranking/density tuning 입력으로 승격하지 않는다.

retained stdout capture 예시:

```bash
cd /home/et16/work/review_system
set -o pipefail
OPENAI_API_KEY=... bash ops/scripts/smoke_openai_provider_direct.sh --expect-live-openai \
  | tee docs/baselines/review_bot/direct_openai_smoke_$(date -u +%F).txt
```

local backend direct smoke capture 예시:

```bash
cd /home/et16/work/review_system
set -o pipefail
OPENAI_API_KEY=placeholder BOT_OPENAI_BASE_URL=http://127.0.0.1:11434/v1 BOT_OPENAI_MODEL=local-model \
  bash ops/scripts/smoke_openai_provider_direct.sh --expect-live-openai \
  | tee docs/baselines/review_bot/direct_openai_smoke_openai_compatible_local_$(date -u +%F).txt
```
...
non-default `BOT_OPENAI_BASE_URL`을 쓴 경우 artifact filename도 아래처럼 분리한다.
...
- `capture_success`: expected endpoint/model provenance가 artifact에 있고, provider quality는
  `status=passed`, direct smoke는 위 direct provider success 조건을 만족한다.
- `human_review_required`: comparison artifact의 `human_review_required=true`, failed/missing comparable
  case, 또는 rubric상 claim strength/evidence anchoring 차이가 있어 사람이 decision artifact를 남겨야 한다.
- `defer`: `skipped`, direct provider failure, provenance mismatch, local backend stdout만 있고
  provider-quality JSON이 없는 경우다. 이 상태에서는 prompt/ranking/rule 변경을 시작하지 않는다.
