...
- [review-engine/review_engine/ingest/conflict_resolver.py](/home/et16/work/review_system/review-engine/review_engine/ingest/conflict_resolver.py:1)
  - 특정 family는 무조건 `authoritative`가 되는 분기문이 있다.
...
1. `source_priority.json` 의미를 `priority_policy`로 치환
2. `conflict_rules.json`을 pack-neutral override schema로 변환
3. `conflict_resolver.py`에서 family 하드코딩 제거
