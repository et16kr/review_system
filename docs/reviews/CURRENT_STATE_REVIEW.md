...
다음 review action은 `13. Consolidated Review Outcome`이다. Unit 12에서는 deterministic
gate와 runtime smoke의 분리는 유지하되, documented TestClient-backed gates가 timeout 없이
hang signal을 남기지 못하는 문제를 post-review bug-fix 후보로 추가했다. Unit 7과 Unit 9의
direct OpenAI smoke는 configuration에 의해 skipped된 provider evidence limitation으로 남긴다.
