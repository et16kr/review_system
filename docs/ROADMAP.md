...
- private/organization rule packaging을 runtime extension contract 위에 얹는 작은 구현 단위
...
### 1. Add Private Rule Package Manifest Validation

상태: `active`

왜 지금 하나:

- filesystem extension root와 private extension runtime test는 이미 있다.
- package metadata, compatibility, included files, provenance validation은 외부 서비스 없이 deterministic하게
  구현할 수 있다.
- package validation이 먼저 있어야 install/update/rollback slice가 public canonical artifact를 건드리지
  않는지 검증할 수 있다.

이번 작업의 범위:

1. private rule package metadata model을 추가한다.
   - manifest file name은 `package.yaml`을 기본으로 한다.
   - v1 field는 `package_id`, `package_version`, `package_kind`, `schema_version`,
     `compatible_review_engine`, `extension_roots`, `included`, `provenance`로 제한한다.
   - unknown key, unsupported `package_kind`, unsupported `schema_version`, path traversal,
     absolute package-internal path, missing included file은 fail-fast 한다.
2. validation API 또는 CLI를 추가한다.
   - CLI 이름은 기존 패턴에 맞춰 정하되, operator가 package root를 주면 metadata와 runtime
     `manifest.yaml` 경계를 함께 검증할 수 있어야 한다.
   - validation은 package metadata를 runtime rule-root `manifest.yaml` 저장소로 확장하지 않는다.
3. sample package fixture를 최소 하나 추가하거나 기존
   `review-engine/examples/extensions/private_org_cpp/`에 package metadata를 붙인다.
4. deterministic tests를 추가한다.

완료 기준:

- valid private package manifest가 통과한다.
- unknown metadata key, missing included file, path traversal, unsupported kind/version이 실패한다.
- runtime loader의 existing extension root contract가 깨지지 않는다.

검증:

```bash
cd review-engine && uv run pytest tests/test_rule_runtime_private_extension.py tests/test_rule_runtime.py -q
cd review-engine && uv run pytest tests/test_rule_lifecycle_cli.py -q
git diff --check
```

연결 문서:

- [rule_authoring_and_editor.md](/home/et16/work/review_system/docs/deferred/rule_authoring_and_editor.md:66)

### 2. Add Private Rule Package Split Validation Gate

상태: `queued`

왜 지금 하나:

- package metadata validation만으로는 private generated artifact와 public canonical artifact가 분리되는지
  보장하기 어렵다.
- installable package를 구현하기 전에 package-specific strict load, source coverage, public-only runtime
  regression을 한 gate에서 재현해야 한다.

선행 조건:

- `Add Private Rule Package Manifest Validation` 완료

이번 작업의 범위:

1. package validation gate를 추가한다.
   - package-specific strict load
   - source coverage matrix 또는 package source manifest validation
   - private extension runtime retrieval test
   - public-only runtime regression
2. 기존 `ops/scripts/run_review_engine_extension_ci.sh --mode all`과 역할이 겹치면 재사용하거나 얇게 확장한다.
3. package validation summary를 deterministic stdout 또는 retained artifact-ready markdown 형태로 남긴다.
4. public `review-engine/data`와 public Chroma collection 이름을 쓰지 않는지 테스트한다.

완료 기준:

- package fixture가 split validation gate를 통과한다.
- public-only runtime test가 private package 없이도 계속 통과한다.
- private package validation 실패가 package-only failure로 분리되어 보인다.

검증:

```bash
cd review-engine && uv run pytest tests/test_rule_runtime_private_extension.py tests/test_source_coverage_matrix.py -q
bash -n ops/scripts/run_review_engine_extension_ci.sh
git diff --check
```

연결 문서:

- [rule_authoring_and_editor.md](/home/et16/work/review_system/docs/deferred/rule_authoring_and_editor.md:117)

### 3. Add Private Rule Package Install Plan Output

상태: `queued`

왜 지금 하나:

- private package install/update/rollback은 active pointer 전환으로 정의되어 있다.
- 실제 pointer 변경까지 자동화하기 전에, validated package가 어떤 runtime root와 artifact root를 사용해야
  하는지 machine-readable install plan으로 출력하면 운영 contract를 작게 검증할 수 있다.

선행 조건:

- `Add Private Rule Package Manifest Validation` 완료
- `Add Private Rule Package Split Validation Gate` 완료

이번 작업의 범위:

1. validated package root에서 install plan을 출력하는 CLI/API를 추가한다.
2. install plan은 최소한 아래 값을 포함한다.
   - package id/version
   - versioned runtime extension root
   - private artifact root
   - dataset output paths
   - private Chroma collection naming preview
   - operator가 `REVIEW_ENGINE_EXTENSION_RULE_ROOTS`에 넣을 path
3. 이 unit은 public canonical rules/data/vector artifact를 수정하지 않는다.
4. update/rollback은 실제 mutation이 아니라 previous/current pointer plan과 validation message까지만 다룬다.

완료 기준:

- valid package에서 deterministic install plan이 출력된다.
- install plan path가 package id/version을 포함하고 public artifact root를 재사용하지 않는다.
- invalid package는 install plan을 출력하지 않는다.

검증:

```bash
cd review-engine && uv run pytest tests/test_rule_runtime_private_extension.py -q
git diff --check
```

연결 문서:

- [rule_authoring_and_editor.md](/home/et16/work/review_system/docs/deferred/rule_authoring_and_editor.md:111)

### 4. Add Rule Authoring Validation Preview Command
...
- private rule package first slices가 완료되거나 blocked로 기록되어 package boundary가 다시 흔들리지 않는 상태
