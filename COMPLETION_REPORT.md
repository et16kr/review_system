...
- 이 문서는 초기 완료 시점의 historical snapshot이다.
- 문서에 나오는 Altibase/Altidev4 fixture와 구 경로(`app/`, 루트 `examples/` 등)는 현재 shipped 기준과 다를 수 있다.
- 최신 운영/검증 기준은 `AGENTS.md`, `docs/REVIEW_ENGINE_MULTI_LANGUAGE_EXHAUSTIVE_REVIEW.md`,
...
- 실제 내부 C++ 코드베이스를 베이스라인 분석해 컨벤션 핫스팟을 찾는 저장소 스캐너를 제공한다.
- 실제 `altidev4` 코드 excerpt를 repo 내부 예제로 내장해 외부 경로 없이도 주요 패턴 검출과 검색 결과를 검증한다.
- 사내 C++ 저장소 스타일 unified diff fixture를 repo 내부에 내장해 `review_diff` 경로도 외부 의존 없이 검증한다.
...
- `data/organization_coding_convention_rules.json`
...
스캐너는 실제 내부 저장소 `/home/et16/work/altidev4`를 기준으로 실행했다.
...
- 사내 규칙 소스는 원본 PDF가 아니라 Markdown 요약본이다. 원문 전체를 규칙 단위로
...
1. `data/conflict_rules.json`에 사내 규칙과 C++ Core Guideline 충돌 매핑을 더 확장한다.
2. 검색 정밀도가 더 필요하면 현재 해시 기반 로컬 임베더를 더 강한 오프라인 또는 승인된 임베딩 모델로 대체하거나 보완한다.
3. 실제 내부 저장소에서 많이 나온 패턴을 추가 검색 fixture로 승격해 기대 결과 커버리지를 높인다.
