...
- Transaction cleanup: `Begin` / `Commit` paths should keep rollback ownership visible from the start; branch-local `Rollback` calls added later in the function are not the same as an immediate deferred guard.
