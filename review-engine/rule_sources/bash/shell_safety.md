...
- Prefer strict shell options for automation that changes state, especially when pipelines, unset variables, or temporary-file cleanup would otherwise fail silently.
...
- Failure semantics: `set -euo pipefail`, pipeline visibility, and concrete cleanup behavior around `mktemp`/trap ownership.
...
- Keep broader temporary-file cleanup, trap ownership, and environment assumptions visible at the top of the script when the diff does not expose a concrete leak.
