...
def test_language_registry_detects_framework_and_product_profiles() -> None:
    registry = get_language_registry()
    ...
    nextjs = registry.resolve(
        ...
        source_text="export async function POST(request: Request) { return Response.json(await request.json()) }\n",
    )
