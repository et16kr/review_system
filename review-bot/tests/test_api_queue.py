...
def test_gitlab_note_hook_posts_summarize_note_without_enqueue() -> None:
    _reset_db()
    payload = {
        "object_kind": "note",
        "user": {"username": "alice"},
        ...
        "merge_request": {
            ...
            "source_branch": "feature",
            "target_branch": "main",
            "last_commit": {"id": "head-summarize"},
        },
        "object_attributes": {
            "id": 614,
            "note": "@review-bot summarize",
            "noteable_type": "MergeRequest",
            "system": False,
        },
    }
    ...
    session = SessionLocal()
    try:
        with patch.object(api_main.runner, "post_summarize_note", return_value=True) as mock_summarize:
