"""Deterministic ASGI client for API queue tests.

The installed Starlette/AnyIO TestClient path can block in the thread bridge before
app code runs. These tests exercise API contracts, so sync endpoints run inline and
requests go through httpx ASGITransport instead of the blocking bridge.
"""
